...
const Post: FunctionComponent<PostProps> = ({ data, location }) => {
  ...
  const related = data.related.edges.filter(
    (relatedPost) => relatedPost.node.id !== post.id
  )
    ...
    <Layout header={<DefaultHeader />} classNames={[`single-post`]}>
      ...
      <div className={"post"} ref={scroll}>
        <div className={"post__sidebar-left"}>
          <button
            className={`post__toggle-toc-button`}
            onClick={toggleToc}
            role={`button`}
          ...
          <div
            className={`post__toc ${showToc ? "post__toc--show-mobile" : ""}`}
          >
              ...
              <div className={`post__related-posts`}>
                ...
                {related.map(({ node }, index) => (
                  <PostCard
                    post={node}
                    key={index}
                    additionalClassNames={[`post-card--in-sidebar`]}
                  />
                ))}
              </div>
            )}
...
export const query = graphql`
  query($slug: String!, $primaryTagSlug: String!) {
    ...
    related: allGhostPost(
      limit: 3
      filter: { tags: { elemMatch: { slug: { eq: $primaryTagSlug } } } }
    ) {
      edges {
        node {
          ...GhostPostFields
