...
const Search: FunctionComponent = () => {
  ...
  return (
    <>
      <button className={`navigation__item-link toggle-search`} onClick={() => toggleSearch()} role={`button`}
              aria-label={`Search`}>
        <FaSearch/>
      </button>
      {isOpen &&
      <div className={`search ${isOpen ? 'search--open' : ''}`} ref={searchRef}>
        <input
          type={`text`}
        ...
        <h5 className={`search__results-title`}>Results ({results.length})</h5>

        <ul className={`search__result-list`} ref={resultListRef}>
          {results.map((item, index) => (
            <li className={`search__result ${selected === index ? 'search__result--selected' : ''}`}
